...
typedef uint64_t h_magic_t;
typedef uint64_t h_size_t;
typedef uint64_t h_flag_t;
...
typedef enum  flash_config_t {
	...
	MagicSig     = 0xaa553cc3aa553cc3,
	BlankSig     = 0xffffffffffffffff
} flash_config_t;

typedef enum  flash_flags_t {
	SizeMask      = 0x0000000000000003,
	MaskEntry     = ~SizeMask,
	BlankEntry    = (h_flag_t)BlankSig,
	ValidEntry    = (0xa5aca5aca5aca5ac & ~SizeMask),
	ErasedEntry   = 0x0000000000000000,
} flash_flags_t;
...
typedef struct flash_entry_header_t {
	h_magic_t            magic;           /* Used to ID files 32*/
	h_flag_t             flag;            /* Used to erase this entry 64 */
	uint64_t             crc;             /* Calculated over the size - end of data 32 */
	h_size_t             size;            /* When added to a byte pointer to flash_entry_header_t
                                               * Will result the offset of the next active file or
                                               * free space. 64 */
	flash_file_token_t   file_token;      /* file token type - essentially the name/type 32 */
...
int16_t up_progmem_write(uint32_t addr, const void *buf, int32_t count)
{
	__disable_irq();
    uint8_t *hword = (uint8_t *)buf;

    if(HAL_FLASH_Unlock() != HAL_OK) {
		Info_Debug("Flash Unlock error\n");
	}
    ...
    for(int32_t i=0; i<count; i+=8) {
        HAL_StatusTypeDef ret = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, (uint32_t)(addr+i), *(uint64_t*)(hword+i));
		if(ret == HAL_OK) {
            if((*(uint64_t *)(addr+i)) != *(uint64_t*)(hword+i)) {
                Info_Debug("Flash write error\n");
                break;
            }
        } else {
			Info_Debug("Flash %d write error-%d-%d\n", count, ret, HAL_FLASH_GetError());
            break;
        }
    }

	if(HAL_FLASH_Lock() != HAL_OK) {
		Info_Debug("Flash Lock error\n");
	}
	__enable_irq();
    return count;
}
...
static inline int blank_flash(uint64_t *pf)
{
	return *pf == BlankSig;
}
...
static int erase_sector(sector_descriptor_t *sm, flash_entry_header_t *pf)
{
	__disable_irq();
	int rv = 0;
	uint32_t page = GetPage((uint32_t)pf);
	uint32_t bank = GetBank((uint32_t)pf);
	...
	if (page > 0 && page == sm->page) {
        ...
        if (HAL_FLASHEx_Erase(&pEraseInit, &SectorError) != HAL_OK)
        {
            ...
            rv = -1;
			Info_Debug("Flash Erase error-%d\n", SectorError);
        }
...
static int erase_entry(flash_entry_header_t *pf)
{
	...
	h_size_t size = sizeof(h_flag_t);
	int rv = up_progmem_write((h_size_t) &(pf->flag), &data, size);
...
int parameter_flashfs_alloc(flash_file_token_t token, uint8_t **buffer, size_t *buf_size)
{
	...
	if (sector_map) {
		...
		if (!working_buffer_static) {
			...
			// 分配uint64对齐
			size_t len = (working_buffer_size + 7)/8;
			working_buffer_size = len * 8;
...
static sector_descriptor_t params_sector_map[] = {
	{118, 2 * 1024, ADDR_FLASH_PAGE_118},  // 2K Bytes
	{119, 2 * 1024, ADDR_FLASH_PAGE_119},  // 2K Bytes
	{120, 2 * 1024, ADDR_FLASH_PAGE_120},  // 2K Bytes
	{121, 2 * 1024, ADDR_FLASH_PAGE_121},  // 2K Bytes
	{122, 2 * 1024, ADDR_FLASH_PAGE_122},  // 2K Bytes
	{123, 2 * 1024, ADDR_FLASH_PAGE_123},  // 2K Bytes
	{124, 2 * 1024, ADDR_FLASH_PAGE_124},  // 2K Bytes
	{125, 2 * 1024, ADDR_FLASH_PAGE_125},  // 2K Bytes
	{126, 2 * 1024, ADDR_FLASH_PAGE_126},  // 2K Bytes
	{127, 2 * 1024, ADDR_FLASH_PAGE_127},  // 2K Bytes
    {0, 0, 0},
};
