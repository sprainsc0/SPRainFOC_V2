...
static int param_import_callback(bson_decoder_t decoder, void *private, bson_node_t node)
{
	...
	if (param == PARAM_INVALID) {
		Info_Debug("ignoring unrecognised parameter '%s'\n", node->name);
		return 1;
	}
	...
	case BSON_INT32:
		if (param_type(param) != PARAM_TYPE_INT32) {
			Info_Debug("unexpected type for %s\n", node->name);
	...
	case BSON_BINDATA:
		...
		if (tmp == NULL) {
			Info_Debug("failed allocating for '%s'\n", node->name);
			goto out;
		}

		if (bson_decoder_copy_data(decoder, tmp)) {
			Info_Debug("failed copying data for '%s'\n", node->name);
			goto out;
		}
	...
	if (!param_set_external(param, v, state->mark_saved, true)) {
		...
		Info_Debug("error setting value for '%s'\n", node->name);
		goto out;
	}
...
static int param_import_internal(bool mark_saved)
{
	...
	uint8_t *buffer = 0;
	size_t buf_size;
	if(parameter_flashfs_read(parameters_token, &buffer, &buf_size) < 0) {
		Info_Debug("flash read error \n");
	}
	...
	while ((s = (struct param_wbuf_s *)utarray_next(param_values, s)) != NULL) {
		...
		case PARAM_TYPE_INT32:
			...
			if (bson_encoder_append_int(&encoder, param_name(s->param), i)) {
				Info_Debug("BSON append failed for '%s'\n", param_name(s->param));
				goto out;
			}
