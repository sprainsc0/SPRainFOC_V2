...
{
	const float L = (3.0f / 2.0f) * _obs_cfg.motor_l;
	float R = (3.0f / 2.0f) * _obs_cfg.motor_r;

	// Saturation compensation
	const float sign = (motor->i_q * motor->v_q) >= 0.0f ? 1.0f : -1.0f;
	R -= R * sign * _obs_cfg.foc_sat_comp * (motor->i_abs_filter / _obs_cfg.l_current_max);

	// Temperature compensation
	const float t = motor->temp_motor;
	if (_obs_cfg.foc_temp_comp && t > -25.0f) {
		R += R * 0.00386f * (t - _obs_cfg.foc_temp_comp_base_temp);
	}
	...
	// TODO
	const float gamma_half = _obs_m.m_gamma_now * 0.5f; 
...
void Observer::observer_task(volatile foc_status_s *motor)
{
	...
	_obs_m.m_gamma_now = gamma_tmp * 4.0f;
}
