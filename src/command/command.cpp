        ...
        if (cmd_updated) {
            ...
            case CMD_PRE_CALIBRATION:
                if (_command.sub_cmd == 1) {
                    ...
                    ind->do_calibration(_command.param1, (int)_command.param2);
                    delete ind;
                    exit_cali_mode();
                    ...
                    delete res;
                    exit_cali_mode();
                } else if (_command.sub_cmd == 6) {
                    enter_cali_mode();
                    lhall = new Enc_LHALL(_cal_status_pub);
                    lhall->do_calibration(_command.param1, (uint8_t)_command.param2);
                    delete lhall;
                    exit_cali_mode();
                }
                break;
...
int Commander_main(int argc, char *argv[])
{
    ...
    for(int i=0; i<argc; i++) {
        ...
        // cmd lhall 0.6 1
        if (!strcmp(argv[i], "lhall")) {
            if (argc < 3) {
                Info_Debug("input argv error\n");
                Info_Debug("cmd lhall [torque] [use_current]\n");
                return 0;
            }
            struct foc_command_s command;
            orb_advert_t cmd_pub = ipc_active(IPC_ID(foc_command), &command);
